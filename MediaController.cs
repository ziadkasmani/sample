...
namespace HeyVoteWeb.Controllers
{
    public class MediaController : ApiController
    {
        ...
        [HttpGet]
        public HttpResponseMessage Play(string id, string ff)
        {
            // This can prevent some unnecessary accesses. 
            // These kind of file names won't be existing at all. 
            if (string.IsNullOrWhiteSpace(id) || string.IsNullOrWhiteSpace(ff))
                throw new HttpResponseException(HttpStatusCode.NotFound);

            //FileInfo fileInfo = new FileInfo(Path.Combine(InitialDirectory, f));
            FileInfo fileInfo = new FileInfo(String.Format(@"{0}/{1}/{2}.mp4", AppConfigManager.HeyFolderPath, ff, id));
        ...
        [HttpGet]
        public HttpResponseMessage PlayAudio(string id, string ff)
        {
            // This can prevent some unnecessary accesses. 
            // These kind of file names won't be existing at all. 
            if (string.IsNullOrWhiteSpace(id) || string.IsNullOrWhiteSpace(ff))
            ...
            //FileInfo fileInfo = new FileInfo(Path.Combine(InitialDirectory, f));
            FileInfo fileInfo = new FileInfo(String.Format(@"{0}/{1}/{2}.mp3", AppConfigManager.HeyFolderPath, ff, id));
